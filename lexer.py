...
class Token:
    ...
    text: str
    line: int          # 1-based line of first byte
    col: int           # 1-based column of first byte
    subkind: str | None = None  # typename, specifier, statement, numeric, start, end
...
def lex(data: bytes) -> list[list[Token]]:
    ...
    while i <= n:
        ...
        # ASCII check: source is 7-bit ASCII
        ...
        match state:
            case State.START:
                match b:
                    ...
                    case 32 | 9:  # space, tab
                        i += 1
                        col += 1
                    case 13:  # carriage return '\r' (CRLF support)
                        i += 1
            ...
            case State.IDENT:
                if b is not None and (is_alpha(b) or is_digit(b)):
                    ...
                    state = State.START
                    continue  # re-examine current byte in START state
            ...
            case State.NUMBER:
                if b is not None and is_digit(b):
                    ...
                    tokens.append(Token("constant", num_str, start_line, start_col, "numeric"))
                    state = State.START
                    continue  # re-examine current byte in START state
